<?php
// файл описания функцийздесь будут зписаныя вызываемы функции
...
function select_adv_sprav_org_vs_usl ($form_name, $value_default, $add_value)
         {
           ...
           $result= mysql_query($query) or die (mysql_error());
           echo "\n<select size=\"1\" name=\"".$form_name."\" $add_value >\n";
           echo "<option>Выберите организацию</option>";
           while($row= mysql_fetch_array($result))
               {
...
function select_day($name, $default_day)
        {
          if($default_day== "current") {$default_day= date("d");}//добавляем текущую дату
        echo "<select size=\"1\" name=\"".$name."\">\n";
        for($i = 1; $i <= 31; $i++)
           {
...
function select_year ($name, $default_year)
        {
         if($default_year== "current") {$default_year= date("Y");}//добавляем текущую дату
...
function convert_kladr_to_adres($code_street, $nomer_doma, $nomer_korpusa, $nomer_kvartiri)
         ...
         $result= mysql_query($query) or die (mysql_error());
            ...
            if($nomer_doma!="" AND $nomer_doma!="0") echo " Дом ",$nomer_doma;
            if($nomer_korpusa!="" AND $nomer_korpusa!="0") echo " Корпус ",$nomer_korpusa;
            if($nomer_kvartiri!="" AND $nomer_kvartiri!="0") echo " Кв. ", $nomer_kvartiri;
