...
include_once("../classes/database_work.php");
    ...
    function calc_lgot_($range, $id_home, $id_people, $id_uslug)
  	{
  	  echo "<hr>\n calc_lgot_<p>\n";
...
//	    $where4= " AND  lgot_range='1' "; //ищем только локальные льготы//
	    ...
	    $where1= "uslug.id_home='".$id_home."' AND udostover.id_people= '".$id_people."' AND uslug.id_vid_uslug= '".$id_uslug."' ";
	    $where2= " AND udostover.id_vid_udostover= lgot.id_sprav_udostover AND  uslug.id_vid_uslug= lgot.id_sprav_uslug  ";
	    $where3= " AND  lgot_range='2' "; //ищем только глобальные льготы//
	    $where4= " AND  lgot_range='1' "; //ищем только локальные льготы//
...
function koef_lgot($usluga, $house)
	{
	  ...
	  $people= 0;
	    ...
	    while($row= mysql_fetch_array($result))
	    	{
			  $Llgot+= calc_lgot_("LOCAL", $house, $row["id_people"], $usluga);
//			  echo "  local lgot ".$Llgot;
			  $Glgot+= calc_lgot_("GLOBAL", $house, $row["id_people"], $usluga);
			  $people++;
//			  echo " people ".$people." ";			  
			}
